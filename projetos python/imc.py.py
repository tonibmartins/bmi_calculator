...
if (a <= 18.5):
    c = "Baixo Peso"
    idg = "O peso ideal seria {}".format(pidg)
    difig = "O peso recomendado para estar no Peso Normal é de {}".format(g)
elif (18.6 <= a <= 24.9):
    c = "Peso Normal"
elif (25.0 <= a <= 29.9):
    c ="Sobrepeso"
    idp = "O peso ideal seria {}".format(pidp)
    difip = "O peso recomendado para estar no Peso Normal é de {}".format(p)
elif (30.0 <= a <= 34.9):
...
if (confirmation == 1) and (18.6 <= a <= 24.9):
    ...
    print("Certo {}, vamos agora disponibilizar seu resultado".format(user_name))
    print("Seu IMC é de {:.2f} e a categoria é de {} !".format(imc, c))
    print("O peso ideal é de {:.2f} Kg e para estar dentro do Peso Normal deverá perder {:.2f} Kg".format(idp,difip))
